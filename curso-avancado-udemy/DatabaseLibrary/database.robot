...
...             Instale: "pip install -U robotframework-databaselibrary --break-system-packages"
...
...             Para PostgreSQL: "sudo apt install python3-psycopg2"
...
Exemplo 01: Executando scripts SQL via Arquivos
    ...
    Confere se "Lucas Viana" foi inserido com sucesso
...
Conectar no Banco de Dados
    ## PostgreSQL
    Connect To Database   db_module=psycopg2   db_name=robot   db_user=robot   db_password=robot   db_host=localhost   db_port=5432
    ## SQLite 3
      # Connect To Database Using Custom Params   db_module=sqlite3    db_connect_string=database="./${DBName}.db", isolation_level=None
    ## MySQL
      # Connect To Database   db_module=pymysql   db_name=${DBName}   db_user=${DBUser}   db_password=${DBPass}   db_host=${DBHost}   db_port=${DBPort}
    ## SQL Server
      # Connect To Database   db_module=pymssql   db_name=${DBName}   db_user=${DBUser}   db_password=${DBPass}   db_host=${DBHost}   db_port=${DBPort}
    ## Oracle
      # Connect To Database Using Custom Params    db_module=cx_Oracle    db_connect_string="${DBName}/${DBUser}@${DBHost}:${DBPort}/${DBalias}"
